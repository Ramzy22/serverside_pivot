...
class IntelligentPrefetchManager:
    def __init__(self, session_tracker: Any, pattern_analyzer: UserPatternAnalyzer, backend: Any, cache: Any, virtual_scroll_manager: Any = None):
        self.session_tracker = session_tracker
        self.pattern_analyzer = pattern_analyzer
        self.backend = backend # Ibis connection
        self.cache = cache
        self.virtual_scroll_manager = virtual_scroll_manager

    async def prefetch_next_page(self, spec: PivotSpec, start_row: int, end_row: int, expanded_paths: List[List[str]], velocity: float = 0.0):
        """
        Prefetch the next page of data based on current scroll position and velocity.
        This populates the cache used by the virtual scroll manager.
        
        Args:
            velocity: Scroll velocity (e.g. rows per second). High velocity triggers larger prefetch.
        """
        if not self.virtual_scroll_manager:
            return
            
        page_size = end_row - start_row + 1
        
        # Adaptive lookahead based on velocity
        # Base lookahead is 2 pages
        multiplier = 2
        if abs(velocity) > 50: # Threshold for "fast" scroll
            multiplier = 5
        if abs(velocity) > 200: # Very fast scroll
            # Maybe reduce detail or just prefetch aggressively
            multiplier = 8
            
        next_start = end_row + 1
        next_end = next_start + (page_size * multiplier) - 1
        ...
        def fetch():
            ...
            self.virtual_scroll_manager.get_visible_rows_hierarchical(
                spec, next_start, next_end, expanded_paths
            )
            
        await loop.run_in_executor(None, fetch)
        return {"status": "prefetched", "range": (next_start, next_end), "multiplier": multiplier}
