...
class ProgressiveHierarchicalLoader:
    ...
    def __init__(self, backend: IbisBaseBackend, cache, pruning_manager: Optional[HierarchyPruningManager] = None, lock: Optional[threading.Lock] = None):
        self.backend = backend
        self.cache = cache
    ...
    def _create_level_ibis_expression(self, spec: PivotSpec, parent_path: List[str], dimension: str) -> IbisTable:
        ...
        if parent_path:
            ...
            for i, value in enumerate(parent_path):
                ...
                condition = (base_table[field].cast('string') == str(value))
