...
@pytest.fixture
def aggregate_sample_data() -> pa.Table:
    """Fixture with uneven group sizes to validate totals semantics for all aggregate types."""
    return pa.table({
        "region": ["East", "East", "East", "West", "West", "West"],
        "sales": [10, 20, 30, 40, 50, 90],
    })


def _get_single_total_row(result: dict) -> list:
    total_rows = [row for row in result["rows"] if row[0] is None]
    assert len(total_rows) == 1, f"Expected a single totals row, found {len(total_rows)}"
    return total_rows[0]
...
def test_ibis_planner_with_totals_all_aggregates(
    controller: PivotController,
    aggregate_sample_data: pa.Table,
    agg: str,
    alias: str,
    expected_total: float,
):
    """
    Grand totals must use the correct roll-up semantics per aggregate type.
    """
    controller.load_data_from_arrow("sales_aggregates", aggregate_sample_data)

    spec = {
        "table": "sales_aggregates",
        "rows": ["region"],
        "columns": [],
        "measures": [{"field": "sales", "agg": agg, "alias": alias}],
        "filters": [],
        "totals": True,
    }

    result = controller.run_pivot(spec, return_format="dict")
    total_row = _get_single_total_row(result)

    assert total_row[1] == expected_total
